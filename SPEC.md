...
│                            # Loads config/local_modifiers.bzl, applies via set_cfg_modifiers()
│
├── config/                  # Local configuration (machine-specific, gitignored outputs)
│   ├── BUCK                 # Bootstrap toolchain prebuilt cache
│   ├── local_modifiers.bzl.default  # Tracked empty template for USE flag modifiers
│   └── local_modifiers.bzl  # Gitignored — generated by buckos CLI/installer
│                            # Contains LOCAL_MODIFIERS list of constraint_value targets
│
...
### Local Configuration (config/local_modifiers.bzl)

Local USE flag configuration is managed through `config/local_modifiers.bzl` (gitignored).
The root PACKAGE loads this file and applies it via `set_cfg_modifiers()`.

```starlark
# config/local_modifiers.bzl (gitignored, generated by buckos CLI/installer)
LOCAL_MODIFIERS = [
    "//use/constraints:ssl-on",
    "//use/constraints:wayland-on",
    "//use/constraints:systemd-on",
]
```

This file is the Buck2 equivalent of Gentoo's `make.conf USE="..."`. It is:
- Generated by the buckos installer during initial setup
- Managed via `buckos use` CLI commands post-install
- Never tracked in git (machine-specific)

A tracked empty template `config/local_modifiers.bzl.default` is provided.
Run `setup.sh` or copy it manually to get started.

### Per-Package Overrides (PACKAGE files)

Per-package USE flag overrides use `set_cfg_modifiers()` in PACKAGE files
within each package's directory — the proper Buck2 primitive for per-directory
modifier scoping. This is the equivalent of Gentoo's `package.use`.

```starlark
# packages/linux/editors/vim/PACKAGE (gitignored, generated by buckos CLI)
load("@prelude//cfg/modifier:set_cfg_modifiers.bzl", "set_cfg_modifiers")

set_cfg_modifiers(cfg_modifiers = [
    "//use/constraints:python-on",
    "//use/constraints:lua-on",
    "//use/constraints:perl-off",
])
```

Generated per-package PACKAGE files are gitignored via `packages/**/PACKAGE`
in `.gitignore` (the top-level `packages/PACKAGE` for visibility is still tracked).

When no per-package PACKAGE file exists, the parent PACKAGE's modifiers apply
(global defaults from `config/local_modifiers.bzl`).
...
2. **Target modifiers** (`modifiers` attr on rule) — per-target in BUCK file
3. **PACKAGE modifiers** (`set_cfg_modifiers()`) — per-directory tree
4. **Platform defaults** — base configuration

This maps to Gentoo's resolution:
