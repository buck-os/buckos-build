...
def _stabilize_autotools_timestamps(root):
    """Give every file in the build tree one uniform mtime before ``make install``.

    Buck2 materializes the build tree with normalized/equal mtimes, and the
    path/file rewrites the install helper applies perturb a few of them.  Any
    resulting mtime skew makes GNU make think some target is stale during
    ``make install`` and regenerate it -- which repeatedly fails in this
    split-action model:
      * autotools: Makefile/config.h look stale vs config.status -> make
        re-runs config.status / ../configure (the scripts may not resolve).
      * glibc: an included fragment (sysd-sorted, ...) regenerates and, against
        a far-past baseline, sends make into an endless restart loop.
      * file: magic.mgc regenerates by running the fresh file(1) against the
        host's older libmagic ("magic version [545] != [539]") and fails.
      * automake: t/testsuite-part.am (a generated .am) regenerates and fails.
      * gcc: objects look newer than xgcc -> relink race ("xgcc: Permission
        denied").

    Every one of these is caused by NON-uniform mtimes.  Assigning a single
    identical mtime to every regular file makes no target strictly newer than
    any other, so make treats the whole tree as up to date and regenerates,
    reconfigures, recompiles and relinks NOTHING -- it only installs.  (A
    tiered scheme was tried instead but leaks: every generated output that
    depends on a tool/source must be individually classified or it goes stale.
    Uniform is complete.)  Genuinely missing install-only targets are still
    built normally (mtime is irrelevant when the output doesn't exist).
    """
