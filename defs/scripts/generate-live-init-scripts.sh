...
# Do NOT create an empty ld.so.cache here — the rootfs tarball includes
# a valid cache generated by rootfs_helper.py (ldconfig -r).  Clobbering
# it with an empty file prevents the dynamic linker from finding libraries
# in non-default paths (e.g. /usr/lib/systemd for libsystemd-core).
